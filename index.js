...
module.exports = dha = async (dha, mek) => {
	try {
        ...
        const ownerNumber = [`${NOwner}`,`6285893228346@s.whatsapp.net`,`6285893228346@s.whatsapp.net`,`6285893228346@s.whatsapp.net`]
	...
	//		hayuk0 = '[√ VERIFIED]'
      ...
      const hideTag = async function(from, text){
	       ...
	       dha.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/sherlynn.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
      		    ...
      		    const katalog = (teks) => {
             res = dha.prepareMessageFromContent(from, { "orderMessage": { "itemCount": 2021, "message": teks, "footerText": "*BY LoyoBotz*", "thumbnail":fs.readFileSync(`./media/canss.jpg`), "surface": 'CATALOG' }}, {quoted:mek})
			...
			case 'sgif':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Subrek Bro Kenz Channel')
								buffer = fs.readFileSync(ran)
								dha.sendMessage(from, buffer, sticker, {quoted: mek})
								fs.unlinkSync(media)
								fs.unlinkSync(ran)
